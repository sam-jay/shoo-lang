...
let empty_prog _ = assert_equal [] (parse "")

let int_lit _ = assert_equal [Expr(IntLit(5))] (parse "5;")

let arithmetic_lit_test1 _ = assert_equal 
    [Expr(Binop(IntLit(82), Add, IntLit(3)))] (parse "82+3;")

let arithmetic_lit_test2 _ = assert_equal 
    [Expr(Binop(IntLit(82), Sub, IntLit(3)))] (parse "82-3;")

let arithmetic_lit_test3 _ = assert_equal 
    [Expr(Binop(IntLit(82), Mult, IntLit(3)))] (parse "82*3;")

let arithmetic_lit_test4 _ = assert_equal 
    [Expr(Binop(IntLit(82), Div, IntLit(3)))] (parse "82/3;")

let arithmetic_lit_test_mod _ = assert_equal 
    [Expr(Binop(IntLit(14), Mod, IntLit(3)))] (parse "14%3;")

let arithmetic_lit_test5 _ = assert_equal 
    [Expr(Binop(IntLit(82), Add, Binop(IntLit(2), Mult, IntLit(4))))] 
    (parse "82+2*4;")

let float_and_int_add _ = assert_equal
...
let logical_lit_test1 _ = assert_equal 
    [Expr(Unop(Neg, IntLit(4)))] (parse "-4;")

let logical_lit_test2 _ = assert_equal 
    [Expr(Binop(IntLit(5), Greater, IntLit(3)))] (parse "5>3;")

let logical_lit_test3 _ = assert_equal 
    [Expr(Binop(IntLit(1), Or, IntLit(2)))] (parse "1||2;")

let logical_lit_test4 _ = assert_equal 
    [Expr(Binop(IntLit(6), Leq, IntLit(8)))] (parse "6<=8;")

let logical_lit_test5 _ = assert_equal 
    [Expr(Binop(IntLit(3), Less, IntLit(90)))] (parse "3<90;")

let logical_lit_test6 _ = assert_equal 
    [Expr(Binop(IntLit(85), Equal, IntLit(85)))] (parse "85==85;")
    
let logical_lit_test7 _ = assert_equal 
    [Expr(Pop(IntLit(4), Inc))] (parse "4++;")
    
let logical_lit_test8 _ = assert_equal 
...
(* String operations *)
let concatenate_two_strings _ = assert_equal
   [Expr(Binop(StrLit("hi"), Add, StrLit(" world")))]
   (parse "\"hi\" + \" world\";")

let concatenate_three_strings _ = assert_equal
...
let comment_test1 _ = assert_equal [Expr(IntLit(5)); Expr(IntLit(6))] (parse "5;/* this is a \n5; multiline \n comment */6;")

let comment_test2 _ = assert_equal [Expr(IntLit(5)); Expr(IntLit(6))] (parse "5;/* this is a /* once \n 5; nested */ multiline \n comment */6;")

let comment_test3 _ = assert_equal [Expr(IntLit(5)); Expr(IntLit(6))] (parse "5;/* this is a /* /* twice \n 5; */ \n nested */ multiline \n comment */6;")

let linec_test _ = assert_equal [Expr(IntLit(5)); Expr(IntLit(6))] (parse "5; // this is a comment \n 6;")
...
let float_test1 _ = assert_equal [Expr(FloatLit("0.1234"))] (parse "0.1234;")
let float_test2 _ = assert_equal [Expr(FloatLit(".1234"))] (parse ".1234;")
...
let int_dec _ = assert_equal [VDecl(Int, "x", None)] (parse "int x;")
...
let if_elif2_else_true _ = assert_equal 
...
let for_no_increment _ = assert_equal 
    [ForLoop(Some(VDecl(Int, "i", Some(IntLit(0)))), 
        Some(Binop(Id("i"), Less, IntLit(2))),
        None, [Expr(IntLit(5))])] 
    (parse "for (int i = 0; i < 2; ) { 5; }")
    
let for_no_init _ = assert_equal [ForLoop(None, Some(Binop(Id("i"), Less, IntLit(5))),
    Some(Assign(Id("i"), Binop(Id("i"), Add, IntLit(1)))), [Expr(IntLit(5))])] (parse "for ( ; i < 5; i = i + 1) { 5; }")

let for_no_init_no_imp _ = assert_equal [ForLoop(None, Some(Binop(IntLit(3), Less, IntLit(5))), 
  None, [Expr(IntLit(5))])] (parse "for ( ; 3 < 5; ) { 5; }") 

let infinite_loop _ = assert_equal [ForLoop(None, None, None, [Expr(IntLit(5))])] (parse "for (; ; ) {5;}")
...
let enhanced_for_id _ = assert_equal
  [EnhancedFor(Int, "x", Id("foo"), [Expr(IntLit(5))])]
  (parse "for (int x in foo) { 5; }")
let enhanced_for_array_lit _ = assert_equal
...
let struct_declare _ = assert_equal
  [StructDef("Point", [
    (Int, "x", Some(IntLit(0)));
    (Int, "y", Some(IntLit(0)));
    (Int, "z", None) ])]
...
let struct_declare_empty _ = assert_equal
  [StructDef("Empty", [])]
  (parse "struct Empty { }")
...
let destruct _ = assert_equal
  [Expr(Destruct(["x"; "y"; "z"], Id("foo")))]
  (parse "{ x; y; z; } = foo;")

let get_struct_val _ = assert_equal
  [Expr(Dot(Id("myStruct"), "x"))]
  (parse "myStruct.x;")

let set_struct_val _ = assert_equal
...
let one_intarr_decl _ = assert_equal [VDecl(Array(Int), "x", None)] (parse "array<int> x;")
let one_intarr_def _ = assert_equal
...
let explict_def_after_dec _ = assert_equal
...
let two_intarr_decl _ = assert_equal [VDecl(Array(Array(Int)), "x", None)] (parse "array< array<int> > x;")
let two_intarr_def _ = assert_equal
...
let one_intarr_new_def _ = assert_equal 
...
let assign_var_array_access _ = assert_equal
    [Expr(Assign(Id("myVar"), ArrayAccess("x", IntLit(2))))] 
    (parse "myVar = x[2];")

let array_index_assign _ = assert_equal
    [Expr(Assign(ArrayAccess("x", IntLit(2)), IntLit(5)))]
    (parse "x[2] = 5;")

let array_expr_index _ = assert_equal
...
let new_one_array _ = assert_equal [Expr(New(NArray(Int, IntLit(5))))] (parse "new(array<int>[5]);")
let new_struct _ = assert_equal [Expr(New(NStruct("BankAccount")))] (parse "new(BankAccount);")
...
let string_lit_test _ = assert_equal [Expr(StrLit("Hello World"))] (parse "\"Hello World\";")
...
let return_var _ = assert_equal
    [Return(Id("x"))] (parse "return x;")

let return_semi _ = assert_equal
...
let print_test _ = assert_equal
    [Expr(FCall("print", [Id("x")]))]
    (parse "print(x);")

(* TODO: fix the following *)
let no_rec_function_test _ = assert_equal
    [Expr(FExpr({
        name = "";
        params = [(Int, "x"); (Int, "y")];
        typ = Int;
...
let rec_function_test _ = assert_equal
    [Expr(FExpr({
      name = "";
      params = [(Int, "x")];
    typ = Int;
